(function () {
  ...
  function injetarCss() {
    if (document.getElementById("css-orcamentos-pro")) return;
    const st = document.createElement("style");
    st.id = "css-orcamentos-pro";
    st.textContent = `
      .orc-grid{display:grid;grid-template-columns:1.05fr 1.2fr;gap:18px}
      ...
      .quote-items-table input,.quote-items-table select{width:100%}
      ...
      @media (max-width: 1100px){
        ...
        .orc-resumo,.quote-totais{grid-template-columns:repeat(2,1fr)}
      }
    `;
  ...
  async function listarOrcamentos(ctx) {
    ...
    async function carregarDetalhe() {
      ...
      const [itensResp, ticketResp] = await Promise.all([
        ctx.sb.db.from("quote_items").select("id, item_type, description, unit, qty, unit_cost, unit_price, total_cost, total_price, sort_order, status").eq("quote_id", state.selecionado.id).order("sort_order", { ascending: true }),
      ...
      wrap.innerHTML = `
        ...
        <div class="mini-card">
          <div class="mini-card-top">
            <div class="mini-card-title">Resumo do orçamento</div>
            <div>${badgeStatus(state.selecionado.status)}</div>
          </div>
          <div class="mini-card-meta">ID: ${escapeHtml(state.selecionado.id)}</div>
          <div class="mini-card-meta">Ticket: ${escapeHtml(state.selecionado.ticket_id || "—")}</div>
          <div class="mini-card-meta">Versão: ${escapeHtml(state.selecionado.version || 1)}</div>
          <div class="mini-card-meta">Criado em: ${escapeHtml(formatarDataHora(state.selecionado.created_at))}</div>
          <div class="mini-card-meta">Atualizado em: ${escapeHtml(formatarDataHora(state.selecionado.updated_at))}</div>
        </div>

        <div class="mini-card" style="margin-top:12px">
          <div class="mini-card-title">Cliente / chamado</div>
          <div class="mini-card-meta">Cliente: ${escapeHtml(ticket?.client_name || "—")}</div>
          <div class="mini-card-meta">Telefone: ${escapeHtml(ticket?.client_phone || "—")}</div>
          <div style="margin-top:6px">${escapeHtml(ticket?.description || "Sem descrição do chamado.")}</div>
        </div>
      ...
      $("#btnAdicionarItem", wrap).addEventListener("click", () => abrirModalItem(null, state.selecionado.id, carregarDetalhe));
      ...
      $$(".btnEditarItem", wrap).forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          abrirModalItem(item, state.selecionado.id, carregarDetalhe);
        });
      });
      ...
      async function alterarStatus(novoStatus) {
        ...
        if (novoStatus === "approved") {
          ...
          alert("Orçamento marcado como recusado.");
        } else {
    ...
    function abrirModalItem(item, quoteId, onSaved) {
