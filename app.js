(function () {
  ...
  function atualizarBadgeConexao() {
    ...
    if (badge) badge.textContent = window.sb && window.sb.db ? "Conectado" : "Sem conexão";
  }

  function marcarMenuAtivo() {
    const rota = rotaAtual();
    document.querySelectorAll(".nav a").forEach((a) => a.classList.toggle("active", a.getAttribute("data-route") === rota));
  }
  ...
  async function renderizarDashboard() {
    ...
    if (window.ModuloDashboard && typeof window.ModuloDashboard.renderizarDashboard === "function") {
      return window.ModuloDashboard.renderizarDashboard({ areaId: "conteudoTela", sb: window.sb, setErro, setInfo, setTitulo });
    }
    return placeholder("Dashboard", "Módulo não carregado.");
  }
  ...
  async function renderizarTelaAtual() {
    ...
    if (rota === "clientes") {
      ...
      if (
        ...
        typeof window.ModuloClientes.listarClientes === "function" &&
        window.sb &&
        window.sb.db &&
        window.sb.companyId
      ) {
        return window.ModuloClientes.listarClientes({
          areaId: "conteudoTela",
          sb: window.sb,
      ...
      return placeholder("Clientes", "Módulo de clientes não carregado.");
    }
    ...
    if (rota === "chamados") {
      ...
      if (window.ModuloChamados && typeof window.ModuloChamados.listarChamados === "function" && window.sb && window.sb.db && window.sb.companyId) {
        return window.ModuloChamados.listarChamados({ areaId: "conteudoTela", sb: window.sb, companyId: window.sb.companyId, portalToken: window.sb.portalToken || "" });
      }
      return placeholder("Chamados", "Módulo de chamados não carregado.");
    }

    if (rota === "orcamentos") {
      setTitulo("Orçamentos", "Orçamentos com itens de produtos e serviços");
      if (window.ModuloBudgets && typeof window.ModuloBudgets.listarTelaOrcamentos === "function" && window.sb && window.sb.db && window.sb.companyId) {
        return window.ModuloBudgets.listarTelaOrcamentos({ areaId: "conteudoTela", sb: window.sb, companyId: window.sb.companyId, setErro, setInfo, setTitulo });
      }
      return placeholder("Orçamentos", "Módulo comercial não carregado.");
    }

    if (rota === "pipeline") {
      setTitulo("Pipeline Comercial", "Fluxo comercial do chamado ao faturamento");
      if (window.ModuloPipeline && typeof window.ModuloPipeline.listarPipeline === "function" && window.sb && window.sb.db && window.sb.companyId) {
        return window.ModuloPipeline.listarPipeline({ areaId: "conteudoTela", sb: window.sb, companyId: window.sb.companyId, setErro, setInfo, setTitulo });
      }
      return placeholder("Pipeline Comercial", "Módulo de pipeline não carregado.");
    }

    if (rota === "catalogo") {
      setTitulo("Produtos e Serviços", "Cadastro mestre de preços para orçamento dinâmico");
      if (window.ModuloCatalogo && typeof window.ModuloCatalogo.listarCatalogo === "function" && window.sb && window.sb.db && window.sb.companyId) {
        return window.ModuloCatalogo.listarCatalogo({ areaId: "conteudoTela", sb: window.sb, companyId: window.sb.companyId, setErro, setInfo, setTitulo });
      }
      return placeholder("Produtos e Serviços", "Módulo de catálogo não carregado.");
    }
    ...
    if (rota === "ordens") {
      ...
      if (window.ModuloOrdens && typeof window.ModuloOrdens.listarOrdens === "function" && window.sb && window.sb.db && window.sb.companyId) {
        return window.ModuloOrdens.listarOrdens({ areaId: "conteudoTela", sb: window.sb, companyId: window.sb.companyId });
      }
    ...
    if (rota === "compras") {
      ...
      if (window.ModuloCompras && typeof window.ModuloCompras.listarCompras === "function" && window.sb && window.sb.db && window.sb.companyId) {
        return window.ModuloCompras.listarCompras({ areaId: "conteudoTela", sb: window.sb, companyId: window.sb.companyId, workorderId: window.__osSelecionadaId || null });
      }
    ...
    if (rota === "despesas") {
      ...
      if (window.ModuloDespesas && typeof window.ModuloDespesas.listarDespesas === "function" && window.sb && window.sb.db && window.sb.companyId) {
        return window.ModuloDespesas.listarDespesas({ areaId: "conteudoTela", sb: window.sb, companyId: window.sb.companyId });
      }
    ...
    if (rota === "financeiro") {
      ...
      if (window.ModuloFinanceiro && typeof window.ModuloFinanceiro.listarFinanceiro === "function" && window.sb && window.sb.db && window.sb.companyId) {
        return window.ModuloFinanceiro.listarFinanceiro({ areaId: "conteudoTela", sb: window.sb, companyId: window.sb.companyId });
      }
    ...
    if (rota === "agenda") {
      ...
      if (window.ModuloAgenda && typeof window.ModuloAgenda.renderizarAgenda === "function") {
        return window.ModuloAgenda.renderizarAgenda({ areaId: "conteudoTela", sb: window.sb, setErro, setInfo, setTitulo });
      }
    ...
    if (rota === "recorrencia") {
      ...
      if (window.ModuloRecorrencia && typeof window.ModuloRecorrencia.renderizarRecorrencia === "function") {
        return window.ModuloRecorrencia.renderizarRecorrencia({ areaId: "conteudoTela", sb: window.sb, setErro, setInfo, setTitulo });
      }
      return placeholder("Recorrência", "Módulo de recorrência não carregado.");
    }

    if (rota === "configuracoes") {
      return placeholder("Configurações", "Ajustes do sistema.");
    }

    return placeholder("Dashboard", "Tela não encontrada.");
  }
