...
async function sendChamadoMessage() {
  ...
  function ticketRow(t) {
    ...
    // Prioridade visual (sem mexer em boot/rotas/config):
    // - finalizado/cancelado: verde discreto
    // - vencido: vermelho
    // - vence em <= 24h: âmbar
    // - demais: padrão
    const statusNorm = String(t?.status || "").toLowerCase();
    const isDone = statusNorm === "finalizado" || statusNorm === "cancelado";
    let dueAt = null;
    if (t?.due_date) {
    ...
    const now = new Date();
    const isOver = !isDone && dueAt && dueAt.getTime() < now.getTime();
    const hoursToDue = !isDone && dueAt ? (dueAt.getTime() - now.getTime()) / 36e5 : null;
    const isSoon = !isDone && !isOver && hoursToDue !== null && hoursToDue <= 24;
    const prioClass = isDone ? "priority-done" : isOver ? "priority-high" : isSoon ? "priority-med" : "priority-low";
