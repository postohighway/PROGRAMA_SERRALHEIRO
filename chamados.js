(function () {
  ...
  function classeLinhaPrioridade(prioridade) {
    return `ticket-priority-${normalizarPrioridade(prioridade)}`;
  }
  ...
  function obterUrlMidia(path, sb) {
    ...
    try {
      return sb.db.storage.from("tickets-media").getPublicUrl(path)?.data?.publicUrl || "";
    } catch {
      return "";
    }
  }
  ...
  function injetarCss() {
    ...
    st.textContent = `
      ...
      tr.ticket-priority-baixa td:first-child{box-shadow:inset 4px 0 0 #94a3b8}
      tr.ticket-priority-normal td:first-child{box-shadow:inset 4px 0 0 #3d86ff}
      tr.ticket-priority-alta td:first-child{box-shadow:inset 4px 0 0 #f6b73c}
      tr.ticket-priority-critica td:first-child{box-shadow:inset 4px 0 0 #ff5d6c}
    ...
    backdrop.innerHTML = `
      <div class="modal">
        <div class="modal-head">
          <div><div class="modal-title">Novo Chamado</div><div class="panel-sub">Cadastre um novo chamado manualmente.</div></div>
  ...
  async function listarChamados(ctx) {
    ...
    async function carregarLista() {
      ...
      let query = ctx.sb.db.from("tickets").select("id, created_at, client_name, client_phone, description, status, priority, due_date, token, photo1_path, photo2_path, photo3_path, photo4_path, photo5_path, video1_path").eq("company_id", ctx.companyId).order("created_at", { ascending: false });
      if (state.status) query = query.eq("status", state.status);
      if (state.prioridade) query = query.eq("priority", state.prioridade);
      ...
      state.tickets = (data || []).filter(item => !busca || [item.client_name, item.client_phone, item.description, item.status, item.priority].join(" ").toLowerCase().includes(busca));
      ...
      wrap.innerHTML = `<table><thead><tr><th>Criado</th><th>Cliente</th><th>Status</th><th>Prioridade</th><th>Prazo</th><th>Descrição</th><th>Ações</th></tr></thead><tbody>${state.tickets.map(item => `<tr class="${classeLinhaPrioridade(item.priority)}"><td>${escapeHtml(formatarData(item.created_at))}</td><td><div>${escapeHtml(item.client_name || "Sem nome")}</div><div class="muted">${escapeHtml(item.client_phone || "—")}</div></td><td>${badgeStatus(item.status)}</td><td>${badgePrioridade(item.priority || "normal")}</td><td>${escapeHtml(formatarData(item.due_date))}</td><td>${escapeHtml((item.description || "").slice(0,80) || "—")}</td><td><button class="btn btn-secondary btnAbrirChamado" data-id="${item.id}">Abrir</button></td></tr>`).join("")}</tbody></table>`;
  ...
  function abrirModalVisita(ctx, ticket, refreshDetalhe) {
    ...
    $("#salvarModalVisita", backdrop).addEventListener("click", async () => {
      ...
      if (!startAt) {
        erroBox.textContent = "Informe a data e hora da visita.";
        erroBox.classList.add("show");
        return;
      }
      const payload = { company_id: ctx.companyId, ticket_id: ticket.id, event_type: "visit", start_at: new Date(startAt).toISOString(), estimated_minutes: estimated || null, priority, address, notes };
      const ins = await ctx.sb.db.from("schedule_events").insert(payload);
      if (ins.error) {
        erroBox.textContent = ins.error.message || "Falha ao salvar visita.";
        erroBox.classList.add("show");
        return;
      }
      try {
